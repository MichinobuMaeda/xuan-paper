...
const ToggleLanguageButton = () => {
  ...
  const [label, setLabel] = useState(translations[i18n.language]?.label);
  ...
  useEffect(() => {
    ...
    if (savedLanguage && Object.keys(translations).includes(savedLanguage)) {
      ...
      setLabel(translations[savedLanguage]?.label);
    }
  }, []);
  ...
  return (
    <Button
        ...
        label ? (
          <div
            className={`size-6 justify-items-center
              rounded-md outline-2
              outline-light-on-primary dark:outline-dark-on-primary`}
          >
            {label}
          </div>
        ) : (
      ...
      onClick={() => {
        ...
        setLabel(translations[newLanguage]?.label);
      }}
      style="embedded"
    />
  );
